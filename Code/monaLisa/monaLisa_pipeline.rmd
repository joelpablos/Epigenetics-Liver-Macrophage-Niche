...
plotMotifHeatmaps <- function(x,
    ...
    for (i in seq_along(ret)) {
      heatmap_list[[i]] <- Heatmap(ret[[i]], width = unit(width, "inch"))
    }
    combined_heatmap <- draw(heatmap_list, row_gap = unit(1, "mm"))
    show(combined_heatmap)
  }
    
    invisible(ret)
}

# panel.abline(h = 5, identifier = "abline2")panel.abline(h = 5, identifier = "abline2")
m <- top_TFs
# m <- as.matrix(SEtable[1:30,])
plotMotifHeatmaps(x = m,
                  which.plots = c("log2enr", "negLog10Padj"),
                  width = 1.8,
                  maxSig = 10,
                  show_seqlogo = TRUE,
...
```{r}
ggplot(SEtable, aes(x = PercCellsExpr, y = selection.score, color =  log2enr, size = tpm)) +
  geom_point() +
  scale_color_gradientn(colors = c("blue", "red")) +
  labs(x = "PercCellsExpr", y = "selection.score", color = "log2enr", size = "tpm") +
  geom_text(aes(label = rownames(SEtable)), size = 3, vjust = 1.5) +
  theme_bw()
```
...
ggplot(SEtable, aes(x = PercCellsExpr, y = selection.score, color = tpm, size = log2enr)) +
  geom_point() +
  scale_color_gradientn(colors = c("blue", "red")) +
  labs(x = "PercCellsExpr", y = "selection.score", color = "tpm", size = "log2enr") +
  geom_text_repel(aes(label = rownames(SEtable)), size = 3, box.padding = 0.5) +
  theme_bw()
...
ggplot(SEtable, aes(x = PercCellsExpr, y = selection.score, color = tpm, size = log2enr)) +
  geom_point(shape = 21, fill = alpha("white", 0.8), stroke = 0.5) +
  scale_color_gradientn(colors = c("blue", "red")) +
  labs(x = "PercCellsExpr", y = "selection.score", color = "tpm", size = "log2enr") +
  geom_text_repel(aes(label = rownames(SEtable)), size = 3, box.padding = 0.5) +
  theme_bw()
...
ggplot(SEtable, aes(x = PercCellsExpr, y = selection.score, color = tpm, size = log2enr)) +
  geom_point(shape = 21, fill = alpha("cyan", 0.7), stroke = 0.5) +
  scale_color_gradientn(colors = c("blue", "red")) +
  labs(x = "PercCellsExpr", y = "selection.score", color = "tpm", size = "log2enr") +
  geom_text_repel(aes(label = rownames(SEtable)), size = 3, box.padding = 0.5) +
  theme_bw()
...
ggplot(SEtable, aes(x = PercCellsExpr, y = selection.score, fill = tpm, size = log2enr)) +
  geom_point(shape = 21, alpha = 0.7, color = "black", stroke = 0.5) +
  scale_fill_gradientn(colors = c("blue", "red")) +
  labs(x = "PercCellsExpr", y = "selection.score", fill = "tpm", size = "log2enr") +
  geom_text_repel(aes(label = rownames(SEtable)), size = 3, box.padding = 0.5) +
  theme_bw()
...
ggplot(SEtable, aes(x = log2enr, y = selection.score, fill = tpm, size =  PercCellsExpr)) +
  geom_point(shape = 21, alpha = 0.7, color = "black", stroke = 0.5) +
  scale_fill_gradientn(colors = c("blue", "red")) +
  labs(x = "log2enr", y = "selection.score", fill = "tpm", size = "PercCellsExpr") +
  geom_text_repel(aes(label = rownames(SEtable)), size = 4, box.padding = 0.5) +
  theme_bw() +
...
ggplot(SEtable, aes(x = log2enr, y = selection.score, fill = tpm, size = PercCellsExpr)) +
  ...
  scale_fill_gradientn(colors = c("blue", "red")) +
  labs(x = "log2enr", y = "selection.score", fill = "tpm", size = "PercCellsExpr") +
  geom_text_repel(aes(label = rownames(SEtable)), size = SEtable$selection.score*4+3, box.padding = 0.4) +
  theme_bw() +
  scale_x_continuous(limits = c(0, 1), expand = expansion(mult = c(0, 0.1)),
                     breaks = c(0, 0.5, 1), labels = c(0, 0.5, 1)) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.1)))
...
plot <- ggplot(SEtable, aes(x = log2enr, y = selection.score, fill = tpm, size = PercCellsExpr)) +
  ...
  scale_x_continuous(limits = c(0, 1), expand = expansion(mult = c(0, 0.1)),
                     breaks = c(0, 0.5, 1), labels = c(0, 0.5, 1)) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
  ggtitle("monaLisa Hep")  # Add your desired title here



# Save the plot as a high-resolution image
ggsave("../Results/plots/monaLisa_Hep_Thesis.jpeg", plot, height = 12, width = 9, dpi = 300)
...
# Comparison of JAPSAR and CISBP motif databases
```{r}
